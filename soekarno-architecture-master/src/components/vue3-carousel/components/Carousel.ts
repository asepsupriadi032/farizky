...
export default defineComponent({
  ...
  setup(props: CarouselConfig, { slots, emit, expose }: SetupContext) {
    ...
    function updateSlideWidth(): void {
      ...
      if (clientWidth <= rect.width) {
        slideWidth.value = rect.width / config.itemsToShow
      } else {
        slideWidth.value = rect.width / config.itemsToShow / storeDeviceSize.scale
      }
    }
    ...
    const handleDragging = throttle((event: MouseEvent & TouchEvent): void => {
      ...
      const deltaX = (endPosition.x - startPosition.x) / storeDeviceSize.scale
      const deltaY = (endPosition.y - startPosition.y) / storeDeviceSize.scale
