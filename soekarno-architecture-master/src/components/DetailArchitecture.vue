<template>
  <div id="DetailArchitecture">
    <!-- image with svg -->
    <!-- <img src="../assets/dev/detail-arsitektur.jpg" style="position: absolute; top: 0; left: 0; width: 100%; z-index: 100; opacity: 0.3; display: block;"> -->

    <div id="header" class="container">
      <h2 class="roboto-serif-medium text-center mb-0">{{ item.Nama_Karya }}</h2>
      <h3 class="roboto-regular text-center color-black mt-1">
        {{ item.Tahun }}
        {{ item.Tahun && item.Sub_Category ? ' • ' : '' }}
        {{ item.Sub_Category }}
      </h3>
    </div>

    <transition name="slide-up">
      <Carousel v-if="isReady" :wrap-around="true" @click="!activeCarousel ? showModal(0) : false" :style="{
        cursor: activeCarousel ? 'auto' : 'pointer'
      }" :mouseDrag="activeCarousel" :touchDrag="activeCarousel">
        <Slide v-for="(gambar, key) in listGambar" :key="key" :style="{
          pointerEvents: activeCarousel ? 'auto' : 'none'
        }">
          <div class="carousel__item" :style="{ 'background-image': `url(${gambar})` }" @click="showModal(key)"></div>
        </Slide>

        <template v-if="activeCarousel" #addons>
          <Navigation />
          <Pagination />
        </template>
      </Carousel>
    </transition>

    <div id="detailContent" class="container">
      <div class="row">
        <transition name="slide-left">
          <div v-if="isReady" class="cleft col-sm-4 roboto-regular">
            <div class="cleft-item">
              <label class="color-gray">{{ words.lokasi }}</label>
              <p>{{ item.Alamat }}</p>
            </div>
            <div class="cleft-item">
              <label class="color-gray">{{ words.saat_ini }}</label>
              <p>{{ item.Saat_Ini || '-' }}</p>
            </div>
          </div>
        </transition>
        <transition name="slide-right">
          <div v-if="isReady" class="cright col-sm-8">
            <h3>{{ words.tentang }} {{ item.Nama_Karya }}</h3>
            <p class="roboto-regular">{{ item.Caption }}</p>
          </div>
        </transition>
      </div>
    </div>

    <ModelVue :show="isShowModal" @close="isShowModal = false">
      <img v-if="isShowModal" :src="listGambar[showModalIndex]" alt="preview" style="max-width: 100%;">
    </ModelVue>
  </div>
</template> 

<script setup>
import { computed, defineProps, onMounted, ref } from 'vue';
import { Carousel, Navigation, Pagination, Slide } from '../components/vue3-carousel'
import ModelVue from './ModalVue.vue'
import { lang } from '../stores/lang';

const words = computed(() => {
  return lang().words
})
const props = defineProps({
  item: {
    type: Object,
    default: null
  },
  isReady: {
    type: Boolean,
    default: false
  }
})

const isShowModal = ref(false);
const showModalIndex = ref(0);

onMounted(() => {
  document.body.style.overflow = 'auto'
});

const listGambar = computed(() => {
  const list = props.item.List_Gambar
  if (list == undefined || list?.length <= 0) {
    return [
      props.item.Gambar_Preview
    ]
  }
  return list
})
const activeCarousel = computed(() => {
  return listGambar.value?.length > 1
})

function showModal(index) {
  showModalIndex.value = index
  if (listGambar.value[index] == "") return
  isShowModal.value = true
}
</script>

<style lang="scss">
#AppVue {
  background-color: $cream;
}

#DetailArchitecture {
  position: absolute;
  width: 100%;
  background-color: $cream;
  background-image: url('../assets/svg/detail-bg-vector.svg');
  background-position: left 0;
  background-repeat: repeat-y;

  &::before {
    content: "";
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background-image: url('../assets/svg/detail-bg-vector.svg');
    background-position: left -600px;
    background-repeat: repeat-y;
    transform: scaleX(-1);
  }

  #header {
    color: $primary;
    padding-top: 60px;

    h2 {
      max-width: calc(100% - 500px);
      margin: auto;
      letter-spacing: -0.065rem;
    }

    h3 {
      letter-spacing: 0.07rem;
      font-size: 30px;
      line-height: 40px;
    }
  }

  section.carousel {
    .carousel__track {
      filter: none;
    }

    .carousel__item {
      cursor: pointer;
    }
  }

  #detailContent {
    margin-top: 72px;
    color: $black-1;
    font-size: 24px;
    letter-spacing: 0.5px;
    margin-bottom: 60px;

    .row {
      margin: 0 50px;
    }

    .cleft {
      .cleft-item {
        margin-bottom: 20px;

        label {
          margin-bottom: 0px;
        }
      }
    }

    .cright {
      margin-left: -4px;

      h3 {
        font-size: 30px;
        font-weight: 500;
        line-height: 42px;
        letter-spacing: -0.06rem;
        margin-bottom: 17px;
      }

      p {
        font-size: 28px;
        line-height: 42px;
      }
    }
  }
}
</style>